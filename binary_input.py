...
class Binary:
    ...
    def binary_separated_to_base64(self, input_string: str) -> str:
        ascii_result = Binary.binary_separated_to_ascii(
            self,
            input_string)
    ...
    def binary_combined_to_base64(self, input_string: str) -> str:
        ascii_result = Binary.binary_combined_to_ascii(
            self,
            input_string)
        ascii_to_b64 = ascii_result.encode('ascii')
        base64_bytes = base64.b64encode(ascii_to_b64)
    ...
    def binary_converted_separated(self, input_string: str) -> dict:
        ...
        ascii = Binary.binary_separated_to_ascii(
            self,
            input_string)
        sep_results['ASCII'] = f'{ascii}'

        base64 = Binary.binary_separated_to_base64(
            self,
            input_string)
        sep_results['Base64'] = f'{base64}'

        decimal = Binary.binary_separated_to_dec(
            self,
            input_string)
        sep_results['Decimal'] = f'{decimal}'

        hexadecimal = Binary.binary_separated_to_hex(
            self,
            input_string)
        sep_results['Hexadecimal'] = f'{hexadecimal}'

        octal = Binary.binary_seperated_to_octal(
            self,
            input_string)
    ...
    def binary_convert_combined(self, input_string: str) -> dict:
        ...
        ascii = Binary.binary_combined_to_ascii(
            self,
            input_string)
        combined_results['ASCII'] = f'{ascii}'

        base64 = Binary.binary_combined_to_base64(
            self,
            input_string)
        combined_results['Base64'] = f'{base64}'

        decimal = Binary.binary_combined_to_dec(
            self,
            input_string)

        combined_results['Decimal'] = f'{decimal}'
        hexadecimal = Binary.binary_combined_to_hex(
            self,
            input_string)
        combined_results['Hexadecimal'] = f'{hexadecimal}'

        octal = Binary.binary_combined_to_octal(
            self,
            input_string)
    ...
    def print_binary_separated_output_panels(self, input_string: str) -> None:
        sep_results = Binary.binary_converted_separated(
            self,
            input_string=input_string)

        Results.print_results_panels(
            self,
            results_dict=sep_results)
    ...
    def print_binary_combined_output_panels(self, input_string: str) -> None:
        combined_results = Binary.binary_convert_combined(
            self,
            input_string=input_string)
