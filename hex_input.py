...
class Hexadecimal:
    ...
    def hex_to_binary(self, input_string: str) -> str:
        ascii_string = Hexadecimal.hex_to_ascii(
            self,
            input_string)
    ...
    def hex_to_morse_code(self, input_string: str) -> str:
        hex_string = input_string.replace(' ', '')
        morse_code_string = MorseCode.encode_morse_code(
            self,
    ...
    def hex_convert_all(self, input_string: str) -> None:
        ...
        ascii = Hexadecimal.hex_to_ascii(
            self,
            input_string)
        results['ASCII'] = f'{ascii}'

        base64 = Hexadecimal.hex_to_base64(
            self,
            input_string)
        results['Base64'] = f'{base64}'

        binary = Hexadecimal.hex_to_binary(
            self,
            input_string)
        results['Binary'] = f'{binary}'

        decimal = Hexadecimal.hex_to_decimal(
            self,
            input_string)
        results['Decimal'] = f'{decimal}'

        morse_code = Hexadecimal.hex_to_morse_code(
            self,
            input_string)
    ...
    def print_hex_output_panels(self, input_string: str) -> None:
        results = Hexadecimal.hex_convert_all(
            self,
            input_string=input_string)

        Results.print_results_panel(
            self,
