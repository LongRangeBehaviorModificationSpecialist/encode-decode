...
[bold yellow2]{results}''')
    ...
    def print_results_table(self, results_dict: dict) -> None:
        ...
        #               header_style='bold #2070b2',
        #               title=f'\nConvert from {results_dict["type"].upper()} Input',
        #               show_lines=True)

        # table.add_column('Format', justify='left')
        # table.add_column(Text('Encoded String', justify='center'),
        #                  justify='left', ratio=2, no_wrap=False)

        # table.add_row(f'[bold green3]Input Value', f'[bold green3]`{results_dict["input"]}`')

        # new_dict = {key: value for key, value in results_dict.items() if key not in ['type','input']}
        # for key, value in new_dict.items():
        #     table.add_row(f'[bright_white]{key}', f'[khaki1]{value}', end_section=True)
